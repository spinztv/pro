...
class StreamsService(object):
    def __init__(self, addon):
        if int(addon.getSetting('addons.ini.type')) == 0:
            path = xbmc.translatePath(os.path.join('special://profile', 'addon_data', 'plugin.video.spinz', 'addons.ini'))
            xbmc.log('[plugin.video.spinz] Xhoans addons.ini is used', xbmc.LOGDEBUG)
        else:
            customFile = str(addon.getSetting('addons.ini.file'))
            if os.path.exists(customFile):
                path = customFile
            else:
                path = xbmc.translatePath(os.path.join('special://profile', 'addon_data', 'plugin.video.spinz', customFile.split('/')[-1]))
            xbmc.log('[plugin.video.spinz] Custom addons.ini is used: %s' % path, xbmc.LOGDEBUG)
