...
menu_icon = xbmc.translatePath(path+'resources/images/toonget_net.png')
fanart = addon.getAddonInfo('fanart')
icon = addon.getAddonInfo('icon')
addonname = addon.getAddonInfo('name')
base = 'http://www.toonget.net'
...
def MAIN_MENU():
    ...
    for i, link in enumerate(category_links):
        ...
        url = get_url(source='Toonget', endpoint=link)
        xbmcplugin.addDirectoryItem(_handle, url, list_item, True)
    list_item = xbmcgui.ListItem(label='Genres')
    list_item.setArt({'fanart':fanart,'thumb':menu_icon})
    url = get_url(source='Toonget', endpoint=base+'/cartoon-genres')
    xbmcplugin.addDirectoryItem(_handle, url, list_item, True)
    ...
    if pagination is not None:
        ...
        if next_page_li is not None:
            try:
                ...
                list_item.setArt({'fanart':fanart,'thumb':menu_icon})
                url = get_url(source='Toonget', endpoint=next_page_li.a['href'])
                xbmcplugin.addDirectoryItem(_handle, url, list_item, True)
            except:
                pass
...
def SEARCH(title):
    try:
        ...
        if len(series_containers) == 0 and any(x for x in ['&' in title,'%26' in title]):
            ...
            print '=====re-searching for ({}) on ({})====='.format(new_term,'toonget')
            soup = SOUPIFY(query+new_term)
            series_containers = soup.find('div',{'class':'series_list'}).findAll('li')
        
        titles = [regulate.unescape(li.find('div',{'class':'right_col'}).h3.a.text+' [COLOR blue]TOONGET[/COLOR]') for li in series_containers]
        images = [li.find('div',{'class':'left_col'}).a.img['src'] for li in series_containers]
        links = [li.find('div',{'class':'left_col'}).a['href'] for li in series_containers]
