...
def _load_encapsula_resource(sess, response):
    #print 'response.content at beginning of _load_encapsula_resource is\n'+response.content
    if 'bootstrap' in response.content: return response.content
    timing = []
    start = now_in_seconds()
    timing.append('s:{}'.format(now_in_seconds() - start))
    
    try: code = get_obfuscated_code(response.content)
    except: code = response.content
    try: parsed = parse_obfuscated_code(code)
    except: parsed = code
    resource1, resource2 = get_resources(parsed, response.url)[1:]
    #print 'this is resource1\n'+resource1
    sess.get(resource1)

    timing.append('c:{}'.format(now_in_seconds() - start))
    time.sleep(1)  # simulate page refresh time
...
def crack(sess, response):
    ...
    if 'bootstrap' in response.content: 
