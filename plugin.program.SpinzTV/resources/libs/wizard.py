...
def kodi17Fix():
	...
	if DP.iscanceled(): 
		DP.close()
		LogNotify(ADDONTITLE, "[COLOR %s]Enabling Addons Cancelled![/COLOR]" % COLOR2)
		sys.exit()
	DP.close()
	forceUpdate()
...
def disableAll():
	...
	for folder in sorted(addonlist, key = lambda x: x):
		...
		if os.path.exists(addonxml):
			...
			if DP.iscanceled(): 
				DP.close()
				LogNotify(ADDONTITLE, "[COLOR %s]Disabling Addons Cancelled[/COLOR]" % COLOR2)
				sys.exit()
	if len(returnto) > 0:
...
def toggleAdult():
	...
	if count > 0: LogNotify(ADDONTITLE, "[COLOR %s][COLOR %s]%d[/COLOR] Adult Addons %s[/COLOR]" % (COLOR2, COLOR1, count, goto.replace('ing', 'ed'))); forceUpdate(True)
	else: LogNotify(ADDONTITLE, "[COLOR %s]No Adult Addons Found[/COLOR]" % COLOR2)
...
def cleanupBackup():
	if DIALOG.yesno(ADDONTITLE, "[COLOR %s]Would you like to clean up your 'My_Builds' folder?[/COLOR]" % COLOR2, "[COLOR %s]%s[/COLOR]" % (COLOR1, MYBUILDS), yeslabel="[B]Clean Up[/B]", nolabel="[B]No Cancel[/B]"):
		...
		LogNotify(ADDONTITLE, "[COLOR %s]Backup Location: Cleared![/COLOR]" % COLOR2)
	else:
		LogNotify(ADDONTITLE, "[COLOR %s]Clean Up Cancelled![/COLOR]" % COLOR2)
...
def forceUpdate(silent=False):
	...
	if silent == False: LogNotify(ADDONTITLE, 'Forcing Addon Updates')
...
def convertSpecial(url, over=False):
	...
	if over == False: LogNotify(ADDONTITLE, "[COLOR %s]Convert Paths to Special: [COLOR green]Complete![/COLOR][/COLOR]" % COLOR2)
...
def clearCrash():  
	...
	if len(files) > 0:
		if DIALOG.yesno(ADDONTITLE, '[COLOR %s]Would you like to delete the Crash logs?' % COLOR2, '[COLOR %s]%s[/COLOR] Files Found[/COLOR]' % (COLOR1, len(files))):
			for f in files:
				os.remove(f)
			LogNotify('Clear Crash Logs', '%s Crash Logs Removed' % len(files))
		else: LogNotify('Clear Crash Logs Cancelled')
	else: LogNotify('Clear Crash Logs', 'No Crash Logs Found[/COLOR]')
...
def clearPackages(over=None):
	if os.path.exists(PACKAGES):
		try:
			...
			LogNotify(ADDONTITLE,'Clear Packages: [COLOR red]Error![/COLOR]')
...
def clearPackagesStartup():
	...
	if os.path.exists(PACKAGES):
		...
		try:
			...
			if file_count > 0: LogNotify(ADDONTITLE,'Clear Packages: [COLOR green]Success: %s[/COLOR]' % convertSize(cleanupsize))
			else: LogNotify(ADDONTITLE,'Clear Packages: [COLOR red]None Found![/COLOR]')
		except Exception, e:
			LogNotify(ADDONTITLE,'Clear Packages: [COLOR red]Error![/COLOR]')
			log("Clear Packages Error: %s" % str(e), xbmc.LOGERROR)
...
def checkSources():
	if not os.path.exists(SOURCES):
		LogNotify(ADDONTITLE, "No Sources.xml File Found!")
		return False
	x      = 0
	bad    = []
	remove = []
	...
	if len(match) > 0:
		...
		else: LogNotify(ADDONTITLE, "All Sources Are Working")
...
def checkRepos():
	...
	if len(repolist) == 0:
		DP.close()
		LogNotify(ADDONTITLE, "No Repositories Found!")
		return
	...
	if DP.iscanceled(): 
		DP.close()
		LogNotify(ADDONTITLE, "Enabling Addons Cancelled")
		sys.exit()
	DP.close()
	...
	if len(badrepos) > 0:
		...
		LogNotify(ADDONTITLE, "All Repositories Working!")
...
def oldThumbs():
	...
	if len(images) > 0: LogNotify(ADDONTITLE,'Clear Thumbs: [COLOR green]%s Files / %.02f MB[/COLOR]!' % (str(len(images)), removed))
	else: LogNotify(ADDONTITLE,'Clear Thumbs: [COLOR red]None Found![/COLOR]')