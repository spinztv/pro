...
ADDON = xbmcaddon.Addon(id='plugin.program.SpinzTV')
...
class GuideTypes(object):
    ...
    filePath = xbmc.translatePath(os.path.join('special://profile', 'addon_data', 'plugin.program.SpinzTV', 'Itv.ini'))

    def __init__(self):
        try:
            fetcher = FileFetcher('Itv.ini', ADDON)
            ...
            xbmc.log('[plugin.program.SpinzTV] GuideTypes collected: %s' % str(self.guideTypes), xbmc.LOGDEBUG)

            if str(ADDON.getSetting('xmltv.type')) == '':
                ADDON.setSetting('xmltv.type', str(defaultGuideId))
        except:
            print 'unable to parse Itv.ini'
    ...
    def SectionMap(self, section):
        ...
        for option in options:
            try:
                ...
                if dict1[option] == -1:
                    xbmc.log('[plugin.program.SpinzTV] skip: %s' % option, xbmc.LOGDEBUG)
            except:
    ...
    def getGuideDataItem(self, id, item):
        ...
        try:
            ...
            xbmc.log('[plugin.program.SpinzTV] DataItem with index %s not found' % item, xbmc.LOGDEBUG)
        return value


    def getGuideById(self, id):
        xbmc.log('[plugin.program.SpinzTV] Finding Guide with ID: %s' % id, xbmc.LOGDEBUG)
        ret = []
        for guide in self.guideTypes:
            if guide[self.GUIDE_ID] == int(id):
                ret = guide
                xbmc.log('[plugin.program.SpinzTV] Found Guide with data: %s' % str(guide), xbmc.LOGDEBUG)
        return ret
...
if __name__ == '__main__':
    ...
    if ret >= 0:
        ...
        if xbmc.getCondVisibility('system.platform.android') and int(ver) < 15:
            ...
            xbmc.log('[plugin.program.SpinzTV] Running on ANDROID with Kodi v%s --> using workaround!' % str(ver), xbmc.LOGDEBUG)
            filePath = xbmc.translatePath(os.path.join('special://profile', 'addon_data', 'plugin.program.SpinzTV', 'settings.xml'))
